...
//       } catch (err) {
//         console.error(err.message); // Set error if any
...
function Calendar() {
  const [currentDate, setCurrentDate] = useState(new Date());
  const [attendanceData, setAttendanceData] = useState([]); // Store attendance data for all users

  const dayNames = ['SUN', 'MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT'];

  // Fetch attendance data for all users from the backend
  useEffect(() => {
    const fetchAttendanceData = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/allOveeAttendance`);
  ...
  // Function to determine the class for each day's status for multiple users
  const getDayStatusClass = (day, userAttendance) => {
    if (!day) return ''; // If day is null (empty space)

    // Find the user's attendance status for this day
    const status = userAttendance.find(attendance => new Date(attendance.date).toDateString() === day.toDateString());
    if (!status) return ''; // If no attendance data for that day

    // Return corresponding CSS class based on attendance status
    switch (status.status) {
      case 'present':
        return 'present';
      case 'absent':
            ...
            {daysInMonth.map((day, index) => (
              ...
              >
                ...
                <div className='user-attendance'>
                  {attendanceData.map((userAttendance) => (
                    <div key={userAttendance._id} className={`user-status ${getDayStatusClass(day, userAttendance.attendance)}`}>
                      {userAttendance.userName} {/* Display user name */}
                    </div>
                  ))}
                </div>
              </div>
